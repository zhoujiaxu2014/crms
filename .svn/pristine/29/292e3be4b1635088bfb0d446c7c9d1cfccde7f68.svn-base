...
Component({
	...
	methods: {
		...
		initAdards() {
			this.setData({
				activeID:wx.getStorageSync('optionsActiveID')
			})
			_get(`/zuul/customersales/draw-lottery-manage/${this.data.activeID}`).then(({
			// _get(`/zuul/customersales/draw-lottery-manage/${'704344423143571456'}`).then(({
				data: res
			}) => {
				if (res.code == '10000') {
					var that = this
					...
					a.option = '砸金蛋'
					a.awards = []
					let b = ['#EE534F', '#DA70D6', '#FFA827', '#5C6BC0', '#42A5F6', '#66BB6A', '#FFC928', '#AA47BC', '#EC407A',
						'#FF7F50', '#EE534F', '#AA47BC', '#FFC928', '#66BB6A', '#42A5F6', '#5C6BC0', '#EC407A', '#FF7F50'
					]
					res.responseBody.awardList.forEach((item, index) => {
						a.awards.push({
							name: item.awardName,
							color: b[index]
						})
		...
		_zhuan() {
			var that = this;
			let zzx = wx.getStorageSync('asdfasdfsadfqewlfvj')
			if (zzx == 0) {
				wx.showToast({
					title: '没有抽奖次数了哦',
					icon: 'none',
			...
			//开始转
			console.log(that.data.zhuanFLAG)
			// let zzj =	wx.setStorageSync('zhuanFlag',that.data.zhuanFLAG')
			let zzj = wx.getStorageSync('zhuanFlag')
			if(zzj==true){
				that.setData({
					zhuanFLAG:true
				})
			}
			wx.setStorageSync('zhuanFlag',false)
			if(that.data.zhuanFLAG==false){
				console.log('重复提交了')
				return false
			}
			console.log('测试'+that.data.zhuanFLAG)
			//开始转了 把这个变量false
			that.setData({
				zhuanFLAG:false
			})
			...
			_get(`/zuul/customersales/draw-lottery-manage/draw/${that.data.activeID}`).then(({
				data: res
			}) => {
				if (res.code == '3003') {
					// code: "3003"
					...
					that.data.prizeList.awards.forEach((item, index) => {
						console.log(item)
						if (item.name == '谢谢惠顾') {
							zz = index
						}
				...
				if (res.code == '10000') {
					//得到奖品的名称
					//res.responseBody.awardName
					that.data.prizeList.awards.forEach((item, index) => {
						if (res.responseBody.awardName == item.name) {
							zz = index
						}
					})
				} else {
				...
				//>>> 是无符号移位运算符
				var r = zz
				// console.log(that.data.prizeList)
				/*=============不重复抽取=============*/
				if (that.data.repeat) {
				...
				setTimeout(function() {
					...
					//要转多少度deg
					app.runDegs = app.runDegs || 0, app.runDegs = app.runDegs + (360 - app.runDegs % 360) + (2160 - r * (360 /
						awardsConfig.awards.length));
